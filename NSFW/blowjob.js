...
if(!nsfw) {
  ...
  if(respondern == arrayn[2] || respondern == arrayn[3] ) {
    const activar = new Discord.MessageAttachment(linkn, "activar_nsfw.gif")
    await canal.send(respondern, activar)
    return message.delete({timeout: 12000})
  } else {
    await canal.send(respondern).then(e => e.delete({timeout:16000}))
    return message.delete({timeout: 14000})
  }
}



  var link = await loveyou.nsfw("blowjob")
